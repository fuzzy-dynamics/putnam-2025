"""
Numerical verification of Putnam 2025 B2 solution.
...
def compute_x1(f, a=0, b=1):
    ...
    """
    numerator, _ = integrate.quad(lambda x: x * f(x), a, b)
    denominator, _ = integrate.quad(lambda x: f(x), a, b)
...
def compute_x2(f, a=0, b=1):
    ...
    """
    numerator, _ = integrate.quad(lambda x: x * f(x)**2, a, b)
    denominator, _ = integrate.quad(lambda x: f(x)**2, a, b)
    return numerator / denominator
...
def verify_double_integral(f, a=0, b=1):
    ...
    result, _ = integrate.dblquad(integrand, a, b, a, b)
    return result
...
def verify_expansion(f, a=0, b=1):
    ...
    int_xf2, _ = integrate.quad(lambda x: x * f(x)**2, a, b)
    int_f, _ = integrate.quad(lambda x: f(x), a, b)
    int_xf, _ = integrate.quad(lambda x: x * f(x), a, b)
    int_f2, _ = integrate.quad(lambda x: f(x)**2, a, b)
...
def main():
    ...
    # Test 10: Very steep function f(x) = e^(10*x) - 1
    all_passed &= test_function("f(x) = e^(10*x) - 1", lambda x: np.exp(10*x) - 1)
